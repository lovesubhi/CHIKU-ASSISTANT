...
import CHIKU.modules.no_sql.users_db as sql
from CHIKU import (
    BOT_NAME,
    BOT_USERNAME,
    LOGGER,
...
from CHIKU.modules import ALL_MODULES
from CHIKU.modules.helper_funcs.chat_status import is_user_admin
from CHIKU.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("CHIKU.modules." + module_name)
        ...
        if mod_match:
            ...
            query.message.edit_caption(text,
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton(text="ʙᴀᴄᴋ", callback_data="help_back"),InlineKeyboardButton(text="sᴜᴘᴘᴏʀᴛ", callback_data="CHIKUsupport")]]
                ),
            )
...
def CHIKUabout_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "CHIKU":
        uptime = get_readable_time((time.time() - StartTime))
...
def CHIKU_Main_Callback(update: Update, context: CallbackContext):
    query = update.callback_query
...
╚═════════════════╝.""",parse_mode=ParseMode.MARKDOWN,
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(text="• ʙᴀᴄᴋ •", callback_data="Main_help"),InlineKeyboardButton(text="• sᴜᴘᴘᴏʀᴛ •", callback_data="CHIKUsupport")
                    ]
                ]
            ),
            )
...
""",
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(text="• ʙᴀᴄᴋ •", callback_data="Music_"),InlineKeyboardButton(text="sᴜᴘᴘᴏʀᴛ", callback_data="CHIKUsupport")
                    ]
                ]
            ),
        )
                ...
                [
                    [
                        InlineKeyboardButton(
                            ...
                            url="https://t.me/II_ANGEL_II_BOT?startgroup=true"
                            )
                       ]
                ]
                     )
...
""",reply_markup=x,
    ...
    about_callback_handler = CallbackQueryHandler(
        CHIKUabout_callback, pattern=r"CHIKU", run_async=True
    )
    ...
    CHIKU_main_handler = CallbackQueryHandler(
        CHIKU_Main_Callback, pattern=r".*_help",run_async=True)
    ...
    dispatcher.add_handler(CHIKU_main_handler)

...
from CHIKU import LOGGER
from CHIKU import dispatcher as n
from CHIKU.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
...
class MukeshTelegramHandler:
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                LOGGER.debug(
                    f"[CHIKU-Msg] Loaded filter pattern {pattern} for function {func.__name__}"
                )
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            LOGGER.debug(
                f"[CHIKU-Callback] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}"
            )
            return func
